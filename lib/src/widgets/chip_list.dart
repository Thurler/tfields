...
typedef TChipListKey<T> = GlobalKey<TStatefulChipListState<T>>;
...
/// its state.
class TStatefulChipList<T> extends StatefulWidget {
  /// The title that will be displayed before the chips
  final String title;
  ...
  const TStatefulChipList.noDelete({
    required this.title,
    required this.noChipsText,
    required this.dataToString,
    this.sortLogic = TChipSortLogic.text,
  ...
  @override
  State<StatefulWidget> createState() => TStatefulChipListState<T>();
}

/// The actual state for the stateful version of the ChipList
class TStatefulChipListState<T> extends State<TStatefulChipList<T>> {
